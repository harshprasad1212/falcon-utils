...
class SimpleAuthMiddleware(object):
    ...
    def process_request(self, req: object, resp: object) -> object:
        ...
        if auth and len(auth) > 0:
            error, token = self.oath_client.introspection(None, None, auth_token_string, 'access_token')
                if not error:
                    error, oauth_user = self.oath_client.get_user(auth_token=auth_token_string)
                    if not error:
                        return
